...
#include "fbgemm_utils/stochastic_rounding.h"
...
constexpr inline T pad4(T value) {
    // Compute the first multiple of 4 that is greater than or equal to the
    // given value
    //
    ...
    // down to the nearest multiple of 4 by clearing the 2 least significant
    // bits.
    //
    // Example:
    //   pad4(3) = 4
...
Vec4T<dst_t> dequantize_load(const src_t* value) {
...
class WeightRow {
 public:
    ...
    WeightRow(emb_t* const row, cache_t* const cache_row, const uint32_t dim,
        ...
        if constexpr (!std::is_same_v<emb_t, float>) {
            if (stochastic_rounding) {
                stoc_rounding_state_.init(*stochastic_rounding_philox_args,
                                          salt_value);
                // Store the pointer here to avoid an if-else cond during
                // load/store
    ...
    //   //////////////////////////////////////////////////////////////////////////////
    //   // Load 4 elements from the table row at element offset d into a
    //   register
    //   // variable (Vec4T<dst_t>)
    //   //
    //   // If the cache row pointer is valid, then data will be read from the
    //   cache
    ...
    Vec4T<reg_t> load(const int32_t d) const {
        ...
        //
        // Note: This method assumes that reg_t is of higher precision than
        // cache_t and emb_t
        if (cache_row_) {
    ...
    //   //////////////////////////////////////////////////////////////////////////////
    //   // Store regster variable of 4 elements (Vec4T<reg_t>) back into the
    //   table
    //   // into the table row at element offset d
    //   //
    //   // If the cache row pointer is valid, then data will be written to the
    //   cache
    //   // instead of embedding table.
    //   //////////////////////////////////////////////////////////////////////////////

    template <typename dst_t>
    void quantize_store(dst_t* output, const Vec4T<reg_t>& value) {
        if (stoc_rounding_state_ptr_) {
            stochastic_rounding_vector<dst_t, reg_t>(output, value,
                                                     *stoc_rounding_state_ptr_);
        } else {
            nearest_rounding_vector<dst_t, reg_t>(output, value);
        }
    }

    void store(const Vec4T<reg_t>& v, const int32_t d) {
        // Write back weight (high precision) to cache if resident; else write
        // to embedding table.
        //
        // Note: This method assumes that reg_t is of higher precision than
        // cache_t and emb_t
    ...
    //   //
    //   // This computes the address at where the optimizer state is stored
    //   along the
    //   // embedding table row, and returns a reinterpret-casted pointer.  It
    //   takes
    ...
    T* optimizer_state_ptr() const {
        static_assert(std::is_same_v<T, std::remove_cv_t<std::remove_pointer_t<
                                            std::remove_reference_t<T>>>>,
                      "T must be a pure type (no pointers, references, or "
                      "cv-qualifiers)");

        auto d_emb_ = dim_;

        // Compute the offset along the row where the optimizer data is stored.
        // Since elements are fetched in groups of 4, the offset should be at
        // the first multiple of 4 that is greater than or equal to D
        const auto d_opt_ = utils::pad4(d_emb_);

        // Return the address at the first position
        //
        // Note: In TBE SSD, we should only ever be using cache_row_, however,
        // the WeightRow class has been overloaded to be used for both UVM and
        // SSD contexts.
        //
        if (cache_row_) {
