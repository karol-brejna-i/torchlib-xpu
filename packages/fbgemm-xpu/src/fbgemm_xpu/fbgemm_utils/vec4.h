...
struct Vec4T<float> : public Vec4BaseT<float> {
    ...
    void store(float* p) const { *((sycl::float4*)p) = acc; }

    void store(sycl::float4* p) const { *p = acc; }
...
struct Vec4T<at::Half> : public Vec4BaseT<at::Half> {
    ...
    void load(const uint8_t* p) {
        assert(false &&
               "Loading uint8_t into Vec4T<at::Half> is not supported.");
    }
    ...
    void store(uint8_t* p) const {
        assert(false &&
               "Storing uint8_t from Vec4T<at::Half> is not supported.");
    }
...
Vec4T<scalar_t> vec4_acc(const Vec4T<scalar_t>& lhs,
